...
#sendp(Ether(src)/IPv6(src=src_ip)/UDP()/DHCP6_Solicit(trid=int(v_trid,16))/DHCP6OptClientId(duid=a)/DHCP6OptIA_NA(iaid=int(v_iaid,16)),iface='lo')
#time.sleep(1)
...
sendp(Ether(src=src_mac)/IPv6(src=src_ip)/UDP()/DHCP6_Request(),iface='lo')
time.sleep(2)
sendp(Ether(src=src_mac,dst='00:00:00:00:a0:a0')/IPv6(src=src_ip,dst=ip_dst)/ICMPv6EchoReply(),count=5,inter=1,iface='lo')